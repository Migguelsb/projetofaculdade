...
class Telaprincipal(QMainWindow):
    ...
    def adicionar_transacao(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO tbl_financeiro (valor_transacao, desc_transacao, dt_transacao, fluxo_transacao, fk_usuario) VALUES (%s, %s, %s, %s, %s)",
                (valor, descricao, data, fluxo, self.id_usuario)
            )
            conn.commit()
    ...
    def carregar_dados_tabela(self):
        try:
            conn = conectar_banco()
            cursor = conn.cursor()
            cursor.execute("SELECT id_transacao, valor_transacao, desc_transacao, dt_transacao, fluxo_transacao FROM tbl_financeiro ORDER BY dt_transacao DESC")
    ...
    def login(self):
        ...
        try:
            ...
            if resultado:
                ...
                self.tela_principal = Telaprincipal(id_usuario=resultado[0])
                self.tela_principal.show()
                self.close()
            else:
                # Verifica se email ou CPF já estão cadastrados
                cursor.execute("SELECT * FROM tbl_usuarios WHERE email_usuario = %s OR cpf_usuario = %s", (email, cpf))
                existente = cursor.fetchone()

                if existente:
                    QMessageBox.warning(self, "Aviso", "E-mail ou CPF já cadastrado. Tente fazer o cadastro com outros dados.")
                else:
                    cursor.execute(
                    ...
                    self.tela_principal = Telaprincipal(id_usuario=novo_id)
                    self.tela_principal.show()
                    self.close()
